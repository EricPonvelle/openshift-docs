...
= Creating {product-title} clusters using FIPS encryption
...
[role="_abstract"]
Create a {product-title} cluster using Federal Information Processing Standards (FIPS) encryption.

[id="rosa-hcp-creating-cluster-with-fips-encryption-prereqs"]
...
include::modules/creating-cluster-with-fips-encryption.adoc[leveloffset=+2]
