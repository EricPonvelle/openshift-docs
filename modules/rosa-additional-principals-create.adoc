...
Use the `--additional-allowed-principals` argument to permit access through other accounts and roles.
...
----
+
You can use `arn:aws:iam::account_id:root` to approve an account, `arn:aws:iam::account_id:role/role_name` to a approve a specific role, `arn:aws:iam::account_id:user/user_name` to approve an additional user, and `arn:aws:iam::account_id:federated-user/user_name` to approve a federated user.
