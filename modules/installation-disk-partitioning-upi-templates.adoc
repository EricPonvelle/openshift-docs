...
* If container storage is on the root partition:
** Then, ensure that this root partition is mounted with the "pquota" option by including the `rootflags=pquota` option in the grub command line.

* If the container storage is on a partition mounted by "/etc/fstab":
** The following options should be included in the file:
+
[source,terminal]
----
...
* If the container storage is on a partition mounted by "systemd":
** The follow sample spec shows the mount option added:
+
[source,terminal]
----
spec:
