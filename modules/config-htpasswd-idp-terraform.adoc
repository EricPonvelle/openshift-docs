...
* You have installed and configured the latest version of {rosa-cli}.
...
. Create the `htpasswd_idp.tf` file by running either of the following commands:
+
** To create a user with a generated, randomized password, run:
+
[source,terminal]
----
...
** To specify your passwords when creating a user, run:
+
[source,terminal]
----
$ cat<<-EOF>htpasswd_idp.tf
