...
[role="_abstract"]
You can log in to the {rosa-cli} with Red{nbsp}Hat single sign-on or with an offline authentication token. Red{nbsp}Hat recommends using the `rosa` command line tool with Red{nbsp}Hat single sign-on, instead of using an offline authentication token.
